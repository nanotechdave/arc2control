...
class CurveTracerOperation(BaseOperation):
    ...
    def run(self):
        ...
        cellArray = list(self.cells)
        #define input cell as the first cell of the array
        cellIn = list(self.cells)[0]
        cell1 = list(self.cells)[1]
        cell2 = list(self.cells)[2]

        (wIn, bIn) = (cellIn.w, cellIn.b)
        (w1, b1) = (cell1.w, cell1.b)
        (w2, b2) = (cell2.w, cell2.b)
        ...
        for (idx, (vstart, vstop)) in enumerate(ramps):
            ...
            if idx == len(ramps) - 1 or endpoint_inclusive:
                (v, i) = self.do_ramp(wIn, bIn, vstart, st, vstop+st/2, pw, \
